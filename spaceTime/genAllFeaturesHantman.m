function ips = genAllFeaturesHantman(rootdir,varargin)
...
[doforce,dorecurse,framechunksize,frontside,realtime_compatible,delete_frontside_videos,DEBUG] = myparse(varargin,...
  'doforce',false,'dorecurse',true,'framechunksize',[],'frontside',false,'realtime_compatible',false,...
  'delete_frontside_videos',true,'debug',false);
...
if ~frontside && exist(fullfile(rootdir,'movie.avi'),'file') || ...
  ...
  try    
    ...
    if frontside,
      ...
      if delete_frontside_videos && exist(catmovname,'file'),
  ...
  for ndx =1:numel(dd)
    ...
    if isdir(curd)
       genAllFeaturesHantman(curd,varargin{:});
    end
