...
elseif N == 0
    ...
    totprocs = progress(1);  %%%%
    doneprocs = length(progress)-1;%%%%
    if nargout == 0
        perc = sprintf('%3.0f%%', percent); % 4 characters wide, percentage
        tots = sprintf('(%d/%d)',doneprocs,totprocs);
        disp([repmat(char(8), 1, (w+9+length(tots))), char(10), perc, '[', repmat('=', 1, round(percent*w/100)), '>', repmat(' ', 1, w - round(percent*w/100)), ']',tots]);
    end
end
