function [flowftrs,hogftrs,info] = genFeaturesOffline(varargin)
...
[ts,readframeFcns,vidfile,trxfile,ips,face,framechunksize,frontside,...
  dopatchprint,realtime_compatible] = myparse(varargin,...
  'ts',[],...
  'readframeFcns',[],... 
  'vidfile','',...
  'trxfile','',...
  ...
  if first,
    ...
    for wndx = 1:curn,
      ...
      if wndx > 1,
        ...
        % HOF
        [Vx,Vy,~] = optFlowLk(patchPrev,patchCurr,[],3,[],[],[],winrad_optflow);
        M = sqrt(Vx.^2 + Vy.^2);
