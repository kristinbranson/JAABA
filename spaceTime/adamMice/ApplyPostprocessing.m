classdef ApplyPostprocessing
  % XXXAL: rename to PostProcessor
  
  % This class probably makes sense and could potentially be stateful
  ...
  methods (Static)    
    ...
    function posts = PostProcess(curs,postprocessparams,scoreNorm)      
      ...
      if strcmpi(postprocessparams.method,'hysteresis')
        posts = ApplyPostprocessing.ApplyHysteresis(curs,postprocessparams,scoreNorm);
      else
        posts = ApplyPostprocessing.ApplyFiltering(curs,postprocessparams);
      end
      
      posts = ApplyPostprocessing.RemoveSmallBouts(posts,postprocessparams);      
    end     
