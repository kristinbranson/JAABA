...
void usage()
{
  ...
  p("Images must be in PPM, PNG or JPG format. Version 1.2");
  p("")
  p("Options:")
...
//p("    -subref <n=0>              0: denser sampling or 1: not of image1 patches")
  p("    -ngh_rad <n=0>             if n>0: restrict matching to n pixel neighborhood")
...
//p("    -min_level <n=2>           skip maxima in levels [0, 1, ..., n-1]")
  p("    -mem <n=1>                 if n>0: optimize memory footprint")
