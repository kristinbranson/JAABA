...
#define MAT_GET_DOUBLE(var, name) \
	MAT_GET_VARIABLE(tmp, name); \
...
#define MAT_GET_CELL_STRING(arr, var, i, name) \
  tmp = mxGetCell(arr, i); \
  MAT_GET_STRING(var, name)
#define MAT_GET_DOUBLE_ARRAY(var, name, len, firstframe)	\
  if(!tmp) { \
    fprintf(stderr, "Error finding field '%s' in %s\n", name, pname); \
    return false; \
  } \
  if(len && (int)(mxGetM(tmp)*mxGetN(tmp)) < len+((int)firstframe)-1) {	\
  ...
  memcpy(var, ((double*)mxGetData(tmp))+my_max(1,((int)firstframe))-1, len*sizeof(double)); \
  /*mxDestroyArray(tmp);*/
#define MAT_GET_DOUBLE_ARRAY_FIELD(arr, var, name, len, firstframe)	\
  tmp = mxGetField(arr, id, name); \
  MAT_GET_DOUBLE_ARRAY(var, name, len, firstframe);
#define MAT_GET_CELL_DOUBLE_ARRAY_VARIABLE(var, name, len, id) \
  tmp2 = matGetVariable(pmat, name); \
  if(tmp2) tmp = mxGetCell(tmp2, id);	\
  MAT_GET_DOUBLE_ARRAY(var, name, len, 0);	\
  mxDestroyArray(tmp2);
#define MAT_READ_DATA_FILE(var, fname, len, firstframe)		\
  ...
  if((int)(mxGetM(tmp2)*mxGetN(tmp2)) < len+((int)firstframe)-1) {				\
    fprintf(stderr, "Error in %s, data length was %d, expected %d\n", fname, (int)(mxGetM(tmp2)*mxGetN(tmp2)), len); \
    return false; \
  } \
  memcpy(var, ((double*)mxGetData(tmp2))+my_max(1,((int)firstframe))-1, sizeof(double)*len); \
  matClose(pmat); \
  mxDestroyArray(tmp);		 \
  /*mxDestroyArray(tmp2);*/
