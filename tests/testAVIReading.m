function success=testAVIReading()
% Test that reading frames from a .avi movie gives the same result regardless
% of the order in which frames are requested.  get_readframe_fcn reads .avi via
% VideoReader, which may preload or cache to speed out-of-order reads, so a
% frame's content must not depend on what was read before it.  We read 10
% evenly-spaced frames in ascending order, then read the same 10 frames in a
% fixed randomly-shuffled order, and require each shuffled read to be identical
% to the corresponding in-order read.

  % Fix the RNG so the shuffle (and thus the test) is deterministic.
  rng(42) ;

  moviePath = ...
    '/groups/branson/bransonlab/projects/JAABA/test_data/flies_wing/pBDPGAL4U_TrpA_Rig2Plate17BowlA_20110929T143440/clips/Target16_Frames04791to04825.avi' ;
