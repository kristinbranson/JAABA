...
function [data,units] = compute_dangle2corner(trx,n)
...
for i = 1:nflies,
  ...
  if trx(fly).nframes <= 1,
    ...
    data{i} = sign(trx(fly).angle2corner_rect(1:end-1)).*...
      modrange(diff(trx(fly).angle2corner_rect,1,2),-pi,pi)./trx(fly).dt;
  end
end
