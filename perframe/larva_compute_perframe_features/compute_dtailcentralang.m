...
function [data,units]=compute_dtailcentralang(trx,n)
...
%     temp2=bsxfun(@times,absdtailheadang{1,i},cosperp);
    dtailcentralang{1,i}=modrange(diff(trx(larva).tailcentralang),-pi,pi)./trx(larva).dt;
    % KB: this angle distance didn't make sense to me, changed to one that
    % made sense to me
    %dtailcentralang{1,i}=(mod1(trx(larva).tailcentralang(2:end)-trx(larva).tailcentralang(1:end-1),pi)-pi)./trx(larva).dt;
end
