classdef ChangeBehaviorNameDialog < handle
  properties
    figureJLabel
    behaviorName
  ...
  methods
    function self=ChangeBehaviorNameDialog(behaviorName,figureJLabel)
    ...
    function editBoxEdited(self)
      ...
      behaviorNameNew=get(self.editBox,'string');
      if JLabelData.isValidBehaviorName(behaviorNameNew), ...
        self.behaviorName=behaviorNameNew;
        if self.editBoxEnterKeyPressed,
          self.closeAndSendMessageToJLabel();
        end
      else
        set(self.editBox,'string',self.behaviorName);
      end
    ...
    function doneButtonPressed(self)
      ...
      % and self.behaviorName out-of-sync, so there's not much to do here.
      self.closeAndSendMessageToJLabel();
