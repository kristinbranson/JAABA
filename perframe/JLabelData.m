...
%       end
    ...
    % ---------------------------------------------------------------------
    function object = CreateObjectForComputeWindowDataChunk(obj,expi,flies)
    ...
    % training examples by calling PreLoadWindowData on all labeled frames.
      ...
      for expi = 1:obj.nexps,
        ...
        for flyi = 1:Nfliescurr
          ...
          object{flyi} = CreateObjectForComputeWindowDataChunk(obj,expi,flies);
...
%             tscurr = [];
            ...
            tscurr = obj.windowdata.t(idxcurr);  %object{flyi}.windowdata_t_flyndx;
            labelidxStruct = obj_getlabelidx_struct{flyi};
            t0_labelidx = obj_getlabelidx_t0{flyi};
...
%             if isfield(windowdata,'featurenames')
%               parfor_windowdata{perframei}{flyi}.featurenames = windowdata.featurenames;
%             else
%               parfor_windowdata{perframei}{flyi}.featurenames = {};
%             end
        ...
        for flyi = 1:Nfliescurr
          ...
          obj.predictblocks.expi = [obj.predictblocks.expi repmat(expi,1,tmp2)];
          obj.predictblocks.flies = [obj.predictblocks.flies repmat(flies,1,tmp2)];
          obj.predictblocks.t0 = [obj.predictblocks.t0 parfor_predictblocks{1}{flyi}.t0];
          obj.predictblocks.t1 = [obj.predictblocks.t1 parfor_predictblocks{1}{flyi}.t1];

          tmp = [cellfun(@(x) x{flyi}, parfor_windowdata)];
          nframes = size(tmp(1).X,1);
          obj.windowdata.X = [obj.windowdata.X; [tmp.X]];
          obj.windowdata.exp = [obj.windowdata.exp; repmat(expi,nframes,1)];
          obj.windowdata.flies = [obj.windowdata.flies; repmat(flies,nframes,1)];
          obj.windowdata.t = [obj.windowdata.t; tmp(1).t];
          obj.windowdata.labelidx_cur = [obj.windowdata.labelidx_cur; zeros(nframes,1)];
          obj.windowdata.labelidx_new = [obj.windowdata.labelidx_new; tmp(1).labelidx_new];
          obj.windowdata.labelidx_imp = [obj.windowdata.labelidx_imp; tmp(1).labelidx_imp];
          obj.windowdata.labelidx_old = [obj.windowdata.labelidx_old; zeros(nframes,1)];
          obj.windowdata.predicted = [obj.windowdata.predicted; zeros(nframes,1)];
          obj.windowdata.scores = [obj.windowdata.scores; zeros(nframes,1)];
          obj.windowdata.scores_old = [obj.windowdata.scores_old; zeros(nframes,1)];
          obj.windowdata.scores_validated = [obj.windowdata.scores_validated; zeros(nframes,1)];
          obj.windowdata.postprocessed = [obj.windowdata.postprocessed; zeros(nframes,1)];
          obj.windowdata.isvalidprediction = [obj.windowdata.isvalidprediction; false(nframes,1)];
          %obj.windowdata.featurenames = [obj.windowdata.featurenames tmp(1).featurenames];
        end
...
%    end
    ...
    % AL: Don't understand the above comment, the code seems to apply
    % postprocessing to current and loaded scores.
        ...
        for flies = allflies{expii},
          ...
          for ndx = 1:numel(gaps)-1
            curidx = idx(idxorder(gaps(ndx):gaps(ndx+1)-1));
              ...
              obj.predictdata{endx}{flies}(ibeh).cur_pp(curidx) = obj.Postprocess(curs,scoreNorm(ibeh));
            end
          end
        end
      end
      ...
      for expii = 1:numel(expis),
        ...
        for flies = allflies{expii},
          ...
          if any(curt(2:end)-curt(1:end-1) ~= 1)
            ...
            success = false;
            return; % ALTODO this early return has left obj.predictdata in a half-updated state
          end
          curs = obj.predictdata{endx}{flies}(ibeh).loaded(curidx);
          obj.predictdata{endx}{flies}(ibeh).loaded_pp(curidx) = obj.Postprocess(curs,scoreNorm(ibeh));
        end %flies
      end
      end
    end  % method
        ...
        for i=1:nExpsToCheck
          ...
          if strcmpi(fileType,'perframedir'),
            ...
            if iscell(fn),
              fileExists(i,j) = ...
                all(~isinf(fTS)) || ...
                all(cellfun(@(s) exist(s,'file'),fn)); % seems unnecessary, GetFile has already checked for existence
            elseif ischar(fn),
              fileExists(i,j) = ~isinf(fTS) || exist(fn,'file'); % seems unnecessary, GetFile has already checked for existence
            else
              fileExists(i,j) = false;
            end
            if ~fileExists(i,j) && obj.IsRequiredFile(fileType)
...
%       fieldNamesInClassifier = JLabelData.fieldNamesInClassifier;
      ...
      % AL: vectorize classifier fields based on number of behaviors
      % ALTODO: Seems like classifier properties can be
      ...
      assert(iscell(self.classifier) && numel(self.classifier)==nrealbeh);
      self.classifier_params=classifierStuff.trainingParams;
      assert(iscell(self.classifier_params) && numel(self.classifier_params)==nrealbeh);
      
      clsTS = classifierStuff.timeStamp;
      assert(numel(clsTS)==nrealbeh);
      self.classifierTS=clsTS;
      self.confThresholds=classifierStuff.confThresholds;
      assert(numel(self.confThresholds)==self.nbehaviors);
      self.windowdata.scoreNorm=classifierStuff.scoreNorm;
      assert(numel(self.windowdata.scoreNorm)==nrealbeh);
...
%             res = obj.gt_labelfilename;            
    ...
    % 
    % - Get the full path to the file(s) of type fileType for experiment expi. 
    % - Existence of files is checked; if file does not exist, filename==[]
    % and timestamp = -inf
    % - Vector output supported (eg for fileType='score' when using
...
%         flyStats.classifierfilename = '';
    ...
    function scores = NormalizeScores(obj,scores)
      ...
      if isempty(obj.windowdata.scoreNorm) || all(isnan(obj.windowdata.scoreNorm))
      ...
      scoreNorm = obj.windowdata.scoreNorm;
      assert(isequal(obj.ntimelines,size(scores,1),numel(scoreNorm)));
      for iTL = 1:obj.ntimelines
        tfSm = scores(iTL,:)<-scoreNorm(iTL);
        tfLg = scores(iTL,:)>scoreNorm(iTL);
        scores(iTL,tfSm) = -scoreNorm(iTL);
        scores(iTL,tfLg) = scoreNorm(iTL);
        scores(iTL,:) = scores(iTL,:)/scoreNorm(iTL);
      end
    end  % method
