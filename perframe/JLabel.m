...
for i = 1:numel(handles.guidata.axes_previews),
  ...
  switch roitype,
    case 'circle',
      ...
      h(end+1) = patch(roidata(1)+roidata(3)*cos(tmp),...
        roidata(2)+roidata(3)*sin(tmp),[.7,.7,.7],...
        'LineStyle',':','Parent',handles.guidata.axes_previews(i));
    case 'rectangle',
      h(end+1) = patch(roidata(1)+roidata(3)*.5*[-1,-1,1,1,-1],...
        roidata(2)+roidata(4)*.5*[-1,1,1,-1,-1],[.7,.7,.7],...
        'LineStyle',':','Parent',handles.guidata.axes_previews(i));
    case 'point',
  ...
  if ismember(roitype,{'circle','rectangle'}) && ...
    ...
    h(end+1) = text(roidata(1),roidata(2),handles.data.trx.roinames{i},...
