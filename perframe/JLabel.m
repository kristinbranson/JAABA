...
for behaviori = 1:handles.data.nbehaviors
  ...
  idx = (labelidx.vals == behaviori) & ~labelidx.imp;
  curColor = ShiftColor.decreaseIntensity(handles.labelcolors(behaviori,:));
  for channel = 1:3,
    handles.labels_plot.im(1,idx,channel) = curColor(channel);
  end
end
...
handles.flies_extra_marker = 'o';
if isfield(handles.configparams,'plot') && ...
    isfield(handles.configparams.plot,'trx') && ...
    isfield(handles.configparams.plot.trx,'extra_marker'),
  handles.flies_extra_marker = handles.configparams.plot.trx.extra_marker;
end
handles.flies_extra_linestyle = 'o';
if isfield(handles.configparams,'plot') && ...
    isfield(handles.configparams.plot,'trx') && ...
    isfield(handles.configparams.plot.trx,'extra_linestyle'),
  handles.flies_extra_linestyle = handles.configparams.plot.trx.extra_linestyle;
end
...
if handles.data.IsAdvancedMode()
  ...
  handles.togglebutton_label_behaviors(2) = uicontrol('Style','togglebutton',...
    ...
    'ForegroundColor','w','Units','pixels','FontUnits','pixels','FontSize',14,...
    'FontWeight','bold','BackgroundColor',ShiftColor.decreaseIntensity(handles.labelcolors(1,:)),...
  ...
  set(handles.togglebutton_label_behavior1,...
    ...
    'UserData',1);
  handles.togglebutton_label_behaviors(1) = handles.togglebutton_label_behavior1;
  SetButtonImage(handles.togglebutton_label_behaviors(1));
end
...
for i = 2:handles.data.nbehaviors,
  if handles.data.IsAdvancedMode()
    ...
    handles.togglebutton_label_behaviors(2*i) = ...
      ...
      'ForegroundColor','w','Units','pixels','FontUnits','pixels','FontSize',14,...
      'FontWeight','bold','BackgroundColor',ShiftColor.decreaseIntensity(handles.labelcolors(i,:)),...
      'Position',pos,...
      'Callback',get(handles.togglebutton_label_behavior1,'Callback'),...
      'Parent',handles.panel_labelbuttons,...
...
handles.GUIAdvancedMode = handles.data.IsAdvancedMode();
...
%   set(handles.togglebutton_label_unknown,'String','*Label Unknown*');
  ...
  for j = 1:2*handles.data.nbehaviors,
    ...
    if handles.data.IsAdvancedMode() && mod(j,2); 
...
switch eventdata.Key,
  ...
  case handles.label_shortcuts,
    ...
    if buttonNum > 2*handles.data.nbehaviors,
      ...
      if ~handles.data.IsAdvancedMode() && ~mod(buttonNum,2); return; end 
...
curShortcuts = {};
for j = 1:2*handles.data.nbehaviors
  if ~handles.data.IsAdvancedMode() && ~mod(j,2); continue; end
...
%dy_label_select = labelbuttons_pos(2) - select_pos(2) - select_pos(4);
if ~handles.data.IsAdvancedMode() || handles.data.IsGTMode(),
...
if handles.GUIAdvancedMode == handles.data.IsAdvancedMode()
   return;
end
...
if ~isnan(handles.togglebutton_label_behaviors(end))
  button1_pos = get(handles.togglebutton_label_behaviors(end),'Position');
else
  button1_pos = get(handles.togglebutton_label_behaviors(end-1),'Position');
end  
set(handles.togglebutton_label_unknown,'Units','pixels');
unknown_button_pos = get(handles.togglebutton_label_unknown,'Position');
out_border_y = unknown_button_pos(2);
...
if ~handles.data.IsAdvancedMode(),
  ...
  for i = 1:handles.data.nbehaviors,
    ...
    handles.togglebutton_label_behaviors(2*i) = ...
      ...
      'FontWeight','bold','BackgroundColor',ShiftColor.decreaseIntensity(handles.labelcolors(i,:)),...
      'Position',pos,...
      'Callback',get(handles.togglebutton_label_behaviors(1),'Callback'),...
      'Parent',handles.panel_labelbuttons,...
      'Tag',sprintf('togglebutton_label_behavior%d',i),...
      'UserData',2*i);
  end
