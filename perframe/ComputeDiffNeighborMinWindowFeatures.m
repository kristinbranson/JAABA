  ...
  for windowi = windowis',
    ...
    if SANITY_CHECK,
      ...
      if any(isnan(y(end,:)) ~= isnan(res_dumb)),
        fprintf('SANITY CHECK: diff_neighbor_min, trans = none, r = %d, off = %d, nan mismatch\n',r,off);
      else
        fprintf('SANITY CHECK: diff_neighbor_min, trans = none, r = %d, off = %d, max error = %f\n',r,off,max(abs(y(end,:)-res_dumb)));
      end
    ...
    if IDX_ABS > 0,
      ...
      if SANITY_CHECK,
        ...
        if any(isnan(y(end,:)) ~= isnan(res_dumb)),
          fprintf('SANITY CHECK: diff_neighbor_min, trans = abs, r = %d, off = %d, nan mismatch\n',r,off);
        else
          fprintf('SANITY CHECK: diff_neighbor_min, trans = abs, r = %d, off = %d, max error = %f\n',r,off,max(abs(y(end,:)-res_dumb)));
        end
    ...
    if IDX_FLIP > 0 && ~( (r == 0) && (off == 0) && (IDX_ABS > 0) ),
      ...
      if SANITY_CHECK,
        ...
        if any(isnan(y(end,:)) ~= isnan(res_dumb)),
          fprintf('SANITY CHECK: diff_neighbor_min, trans = flip, r = %d, off = %d, nan mismatch\n',r,off);
        else
          fprintf('SANITY CHECK: diff_neighbor_min, trans = flip, r = %d, off = %d, max error = %f\n',r,off,max(abs(y(end,:)-res_dumb)));
        end
