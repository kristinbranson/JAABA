...
set(handles.table,'ColumnName',...
  ...
  sprintf('%s|validation|errors',handles.JLDObj.labelnames{1}),...
  sprintf('%s|validation|errors',handles.JLDObj.labelnames{2}),...  
...
for selExp = 1:handles.JLDObj.nexps
  for flyNum = 1:handles.JLDObj.nflies_per_exp(selExp)
    ...
    if ~isempty(flyStats.validatedErrorsPos),
      tableData{count,12} = flyStats.validatedErrorsPos;
      tableData{count,13} = flyStats.validatedErrorsNeg;
    else
    ...
    if ~isempty(flyStats.one2two),
      tableData{count,14} = flyStats.one2two;
      tableData{count,15} = flyStats.two2one;
    else
      tableData{count,14} = 0;
      tableData{count,15} = 0;
    end
    count = count+1;
  end
end
