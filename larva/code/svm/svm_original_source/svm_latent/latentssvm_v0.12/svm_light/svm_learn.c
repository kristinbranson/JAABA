...
/***********************************************************************/
/*                                                                     */
/*   Modified by Chun-Nam Yu                                           */
/*   For Latent Structural SVM API                                     */
/*   Date: 2.6.09                                                      */
/*   The two functions optimize_to_convergence_sharedslack() and       */
/*   compute_shared_slacks() are modified in this code.                */
/*                                                                     */
/***********************************************************************/
/*                                                                     */
/*   Fixed convergence issue by turning off shrinking in               */
/*   optimize_to_convergence() and                                     */
     ...
     /* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  for(;retrain && (!terminate);iteration++) {
    ...
    //noshrink=0; // commented out by cnyu
    noshrink=1; 
     ...
     /* maxdiff: returns maximum violation of KT-conditions */
{
  ...
  for(;retrain && (!terminate);iteration++) {
    ...
    /* always pick slackset 1 (assume 1-slack) */
    slackset = 1;
    ...
    if (iteration % 101) {
      ...
      /* random working set to avoid getting locked due to numerical instabilities */
      choosenum=select_next_qp_subproblem_rand(label,unlabeled,a,lin,c,totdoc,
					       minl(learn_parm->svm_maxqpsize,
						    learn_parm->svm_newvarsinqp),
					       learn_parm,inconsistent,active2dnum,
					       working2dnum,selcrit,selexam,kernel_cache,key,
     ...
     /* Check KT-conditions */
{
  ...
  for(ii=0;(i=active2dnum[ii])>=0;ii++) {
    ...
    /* this loop should not be entered once we enforce the sum-to-C constraint */
     ...
     /* compute the value of shared slacks and the joint alphas */
{
  ...
  for(jj=0;(i=active2dnum[jj])>=0;jj++) { /* clear slack variables */
    ...
    /* allow slack variables to go negative */
    /*slack[docs[i]->slackid]=0.0;*/
    slack[docs[i]->slackid]=LARGE_NEG_NUMBER;
    /*    alphaslack[docs[i]->slackid]=0.0; */
  }
  /* modified on June 2 2009 */
  /* only works on one-slack */
  if ((i=active2dnum[0])>=0) {
    dist=(lin[i])*(double)label[i];
    target=-(learn_parm->eps-(double)label[i]*c[i]);    
    slack[0] = target-dist;
  }
  /* end modified on June 2 2009 */
