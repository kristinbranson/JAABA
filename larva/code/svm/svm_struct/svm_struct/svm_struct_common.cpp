...
{
  long i;
  char type[100], cname[400], bname[400], load_constraints_fname[400], save_constraints_fname[400];
  FitParams params;
...
int test_main(int argc, const char* argv[], STRUCT_LEARN_PARM *struct_parm, STRUCTMODEL *structmodel, SVMStructMethod *m)
{
  ...
  int num;
  char **test_list = strlen(outfilelist) ? m->load_examples(outfilelist, &num) : NULL;
  if(test_list) assert(num == testsample.n);

  for(i=0;i<testsample.n;i++) {
    t1=get_runtime();
    y=m->classify_struct_example(testsample.examples[i].x,structmodel,struct_parm);
    runtime+=(get_runtime()-t1);

    m->write_label(predfl,y);
    if(test_list)
      m->save_example(y.data, test_list[i]);
