		     ...
		     optimization if not NULL. Array must be of size totdoc. */
{
  ...
  if(kernel_parm->kernel_type == K_LINEAR) {
    /* kernel_cache = NULL; */
  } 
  ...
  if(alpha) {
    ...
    if(kernel_cache && (kernel_parm->kernel_type != K_LINEAR)) {
     ...
     /* model:       Returns learning result (assumed empty before called) */
{
  ...
  /* caching makes no sense for linear kernel */
  if((kernel_parm->kernel_type == K_LINEAR) && (*kernel_cache)) {
     ...
     /* retrain: selects training mode (1=regular / 2=holdout) */
{
  ...
  for(;retrain && (!terminate);iteration++) {
    ...
    if((!retrain) && (inactivenum>0) 
	   ...
	   || (kernel_parm->kernel_type == K_LINEAR))) { 
      if(((verbosity>=1) && (kernel_parm->kernel_type != K_LINEAR)) 
	 || (verbosity>=2)) {
	if(verbosity==1) {
	  printf("\n");
	}
	...
	retrain=identify_one_misclassified(lin,label,unlabeled,totdoc,
      ...
      if(retrain) {
	if(kernel_parm->kernel_type == K_LINEAR) { /* reinit shrinking */
	  learn_parm->epsilon_crit=2.0;
	} 
      }
      if(verbosity>=1) {
     ...
     /* Check KT-conditions */
{
  ...
  if(kernel_parm->kernel_type == K_LINEAR) {  /* be optimistic */
        ...
        minshrink variables can be removed. */
{
  ...
  if((change>=minshrink) /* shrink only if sufficiently many candidates */
    ...
    if(kernel_parm->kernel_type != K_LINEAR) { /*  non-linear case save alphas */
      a_old=(double *)my_malloc(sizeof(double)*totdoc);
 	         ...
 	         values for linear kernel! */
{
  ...
  if(kernel_parm->kernel_type == K_LINEAR) { /* special linear case */
  ...
  if(kernel_parm->kernel_type != K_LINEAR) { /* update history for non-linear */
    for(i=0;i<totdoc;i++) {
...
{                         /* length of the longest support vector. This is */
  ...
  SWORD nullword;          /* origin of the space. */
