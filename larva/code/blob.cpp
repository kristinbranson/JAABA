...
MultiBlobSequence *import_multi_blob_sequence(const char *dir_name, int num_spine_points) {
  ...
  m->num_blobs = 0;
  m->blobs = NULL;
  sprintf(m->fname, "%s/dir.multi", dir_name);
...
void save_multi_blob_sequence(MultiBlobSequence *m, BehaviorGroups *behaviors, int num_model_pts, int num_orientations, int save_blobs)  {
  ...
  FILE *fout = fopen(m->fname, "w");
  for(i = 0; i < m->num_blobs; i++) {
    int j = 0;
    while(m->blobs[i]->fname[j] && m->blobs[i]->fname[j] == m->fname[j])
      j++;
    while(j > 0 && m->blobs[i]->fname[j] != '/' && m->blobs[i]->fname[j] != '\\')
      j--;
    fprintf(fout, "%s\n", m->blobs[i]->fname+j);
    if(save_blobs)
...
MultiBlobSequence *load_multi_blob_sequence(const char *fname, BehaviorGroups *behaviors)  {
  char dir[1000], bname[400], line[1000];
  strcpy(dir, fname);
  ...
  MultiBlobSequence *b = (MultiBlobSequence*)malloc(sizeof(MultiBlobSequence));
  memset(b, 0, sizeof(MultiBlobSequence));
  ...
  while(fgets(line, 999, fin)) {
    ...
    sprintf(bname, "%s/%s", dir, line);
    b->blobs = (BlobSequence**)realloc(b->blobs, sizeof(BlobSequence)*b->num_blobs);
    b->blobs[b->num_blobs++] = load_blob_sequence(bname, behaviors);
  }
  fclose(fin);
...
void StringAppend(char **str, const char *a, int *len) {
  *str = (char*)realloc(*str, *len+(int)strlen(a)+1);
  strcat(*str+(*len), a);
  *len += strlen(a);
}
...
void save_blob_behavior_bouts(const char *fname, BlobSequence *b, BehaviorGroups *behaviors, int is_meta) {
  int max_bouts = 0;
  char *bstr = (char*)malloc(1);
  int len = 0;
  char str[400];
  *str = '\0';
  bstr[0] = '\0';
  for(int beh = 0; beh < behaviors->num; beh++) {
    int i = 0, num_bouts = 0, v, j;
    sprintf(str, "%s\t", behaviors->behaviors[beh].name); StringAppend(&bstr, str, &len);
    while(i < b->num_frames) {
      j = i;
      v = is_meta ? b->frames[j].meta_behaviors[beh] : b->frames[j].behaviors[beh];
      while(i < b->num_frames && (is_meta ? b->frames[i].meta_behaviors[beh] : b->frames[i].behaviors[beh]) == v)
	i++;
      sprintf(str, "%s\t%f\t%f\t#\t",  behaviors->behaviors[beh].values[v].abbreviation, (float)b->frames[j].frame_time, 
	      (float)b->frames[i < b->num_frames ? i : i-1].frame_time);
      StringAppend(&bstr, str, &len);
      num_bouts++;
    }
    StringAppend(&bstr, "\n", &len);
  ...
  FILE *fout = fopen(fname, "w");
  fprintf(fout, "\t");
  for(int i = 0; i < max_bouts; i++) {
    fprintf(fout, "beh\tstart\tend\t#\t");
  }
  fprintf(fout, "\n%s", bstr);
  fclose(fout);
  free(bstr);
...
BehaviorGroups *load_behaviors(const char *fname, const char *classifier_dir) {
  ...
  while(fgets(line, 999, fin)) {
	...
	sscanf(ptr2, "%x", &b->values[b->num_values].color);
      ptr2 = strtok(NULL, "\t");
      if(ptr2)
	sscanf(ptr2, "%s", b->values[b->num_values].regExp);
