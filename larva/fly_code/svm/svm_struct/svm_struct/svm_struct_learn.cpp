...
void find_most_violated_constraint(SVECTOR **fydelta, double *lossval, 
				   ...
				   long *argmax_count, SVMStructMethod *m, LABEL *ybar_out=NULL);
...
CCACHE *create_constraint_cache(SAMPLE sample, STRUCT_LEARN_PARM *sparm, 
				STRUCTMODEL *sm, SVMStructMethod *m);
...
void svm_learn_struct(SAMPLE sample, STRUCT_LEARN_PARM *sparm,
		      ...
		      STRUCTMODEL *sm, int alg_type, SVMStructMethod *m)
{
  int         i,j;
  int         numIt=0;
  long        argmax_count=0;
  ...
  m->init_struct_model(sample,sm,sparm,lparm,kparm); 
  ...
  cset=m->init_struct_constraints(sample, sm, sparm);
  if(cset.m > 0) {
  ...
  do { /* iteratively increase precision */
	...
	for(i=0; i<n; i++) { /*** example loop ***/
	  ...
	  if((!use_shrinking) || (opti[i] != opti_round)) {
	    ...
	    lossval=m->loss(ex[i].y,ybar,sparm);
	...
	rt_total+=MAX(get_runtime()-rt1,0);
	  ...
	  || m->finalize_iteration(ceps,0,sample,sm,cset,alpha,sparm));  
  ...
  m->print_struct_learning_stats(sample,sm,cset,alpha,sparm);
			    ...
			    STRUCTMODEL *sm, int alg_type, SVMStructMethod *m)
{
  ...
  */
  ...
  if(alg_type == ONESLACK_DUAL_CACHE_ALG) {
    ccache=create_constraint_cache(sample,sparm,sm, m);
    /* NOTE:  */
    for(i=0;i<n;i++) 
      if(m->loss(ex[i].y,ex[i].y,sparm) != 0) {
...
omp_lock_t lock;
omp_init_lock (&lock);
#pragma omp parallel for private(i,rt1,fydelta,rhs_i,rt_viol,rt_psi,argmax_count,ybar)
#endif
	for(i=0; i<n; i++) {
	  rt1=get_runtime();
	  ...
	  if(strlen(m->save_constraints_file()) && !m->empty_label(ybar)) {
#ifdef USE_OPENMP
	    omp_set_lock(&lock);
#endif

	    FILE *fout = fopen(m->save_constraints_file(), "a");
	    m->write_label(ybar, fout, numIt, n);
	    fclose(fout);
#ifdef USE_OPENMP
	    omp_unset_lock(&lock);
#endif	    
	...
	if((alg_type == ONESLACK_DUAL_ALG) 
	   || (alg_type == ONESLACK_DUAL_CACHE_ALG))
	  kparm->kernel_type=K_GRAM; /* use kernel stored in kparm */
	svm_learn_optimization(cset.lhs,cset.rhs,cset.m,sizePsi,
			       lparm,kparm,NULL,svmModel,alpha);
	...
	printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f)\n",cset.m,
      ...
      m->on_finished_iteration(cset, sm, sparm, numIt);

  } while(cached_constraint || (ceps > sparm->epsilon) || 
	  m->finalize_iteration(ceps,cached_constraint,sample,sm,cset,alpha,sparm)
	 );
...
void find_most_violated_constraint(SVECTOR **fydelta, double *rhs, 
				   ...
				   STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm,
				   double *rt_viol, double *rt_psi, 
				   long *argmax_count, SVMStructMethod *m, LABEL *ybar_out)
     ...
     /* create new constraint cache for training set */
{
  ...
  for(i=0;i<n;i++) { 
    ...
    ccache->constlist[i]->rhs=m->loss(ex[i].y,ex[i].y,sparm)/n;
	...
	run. */
{
  ...
  if(viol_gain > gainthresh) {
    fydelta_new=fydelta;
    if(struct_verbosity>=2) rt2=get_runtime();
    if(svmModel->kernel_parm.kernel_type == K_LINEAR) {
...
double find_most_violated_joint_constraint_in_cache(CCACHE *ccache, double thresh, double *lhs_n, SVECTOR **lhs, double *rhs, SVMStructMethod *m)
