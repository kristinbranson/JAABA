     ...
     /* classifies one example */
{
  ...
  if((model->kernel_parm.kernel_type == K_LINEAR) && (model->lin_weights))
     ...
     /* calculate the kernel function */
{
  ...
  if(kernel_parm->kernel_type == K_GRAM) {  /* use value from explicitly */
     ...
     /* calculate the kernel function between two vectors */
{
  ...
  switch(kernel_parm->kernel_type) {
    ...
    case K_SIGMOID:/* sigmoid neural net */
            return(tanh(kernel_parm->coef_lin*sprod_ss(a,b)+kernel_parm->coef_const)); 
    case K_CUSTOM: /* custom-kernel supplied in file kernel.h*/
            return(custom_kernel(kernel_parm,a,b)); 
    default: printf("Error: Unknown kernel function\n"); exit(1);
  }
}

SVECTOR *create_svector(SWORD *words,const char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (SWORD *)my_malloc(sizeof(SWORD)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
  vec->twonorm_sq=-1;
...
SVECTOR *create_svector_shallow(SWORD *words,char *userdefined,double factor)
...
SVECTOR *create_svector_n(double *nonsparsevec, long maxfeatnum, const char *userdefined, double factor)
{
  return(create_svector_n_r(nonsparsevec,maxfeatnum,userdefined,factor,0));
}

SVECTOR *create_svector_n_r(double *nonsparsevec, long maxfeatnum, const char *userdefined, double factor, double min_non_zero)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  for(i=1;i<=maxfeatnum;i++)  {
    //fprintf(stderr, "%d %f\n", (int)i, (float)nonsparsevec[i]);
    if((nonsparsevec[i]<-min_non_zero) || (nonsparsevec[i]>min_non_zero))
      fnum++;
  }
  vec = (SVECTOR *)my_malloc(sizeof(SVECTOR));
  vec->words = (SWORD *)my_malloc(sizeof(SWORD)*(fnum+1));
  fnum=0;
  for(i=1;i<=maxfeatnum;i++) { 
    if((nonsparsevec[i]<-min_non_zero) || (nonsparsevec[i]>min_non_zero)) {
	...
	SVECTOR is used */
{
    ...
    sum=(SWORD *)my_malloc(sizeof(SWORD)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
{
  SVECTOR *sum,*f;
  SWORD    empty[2],*ai,*concat,*concati,*concat_read,*concat_write;
  long    length,i;
  double  weight;
  ...
  if(a){
    ...
    concat=(SWORD *)my_malloc(sizeof(SWORD)*(length+1));
    concati=concat;
    for(f=a;f;f=f->next) {
      ai=f->words;
      while (ai->wnum) {
    ...
    qsort(concat,length,sizeof(SWORD),compareup_word);
...
double sprod_nn_map(double *vec_n, double *vec_s, int n, bool *map) {
  int i;
  double sum = 0;
  for(i = 0; i < n; i++) {
    if(map[i])
...
void print_percent_progress(long *progress, long maximum, 
			    long percentperdot, const char *symbol)
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  if(0 && model->kernel_parm.kernel_type == K_LINEAR) {
    if(verbosity>=1) {
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  words = (SWORD *)my_malloc(sizeof(SWORD)*(max_words_doc+10));
  if(verbosity>=1) {
...
int parse_document(char *line, SWORD *words, double *label,
