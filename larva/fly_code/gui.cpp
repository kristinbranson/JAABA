...
char g_processFile[400], g_processDir[400];
                      ...
                      wxTAB_TRAVERSAL)
{
  ...
  timeline_zoom = 1;
    ...
    */
    ...
    m_timeline_sizer2 = new wxFlexGridSizer(behaviors->num+1,3,0,0);   
...
void LabelFrame::OnFitFrame(wxCommandEvent& WXUNUSED(event)) {
  ...
  compute_global_features(blobs, frame, &params, 1);
...
void LabelFrame::OnSize(wxSizeEvent &event) {
  wxSize sz = GetSize();
  timeline_splitter->SetSashPosition(sz.y-TIMELINE_PANEL_HEIGHT-TIMELINE_HEIGHT*(behaviors->num+1));
  ...
  img_window->SetMinSize(wxSize(sz.x-CONTROL_WIDTH,sz.y-TIMELINE_PANEL_HEIGHT-TIMELINE_HEIGHT*(behaviors->num+1)));
  mutex->Lock();
  if(blobs) {
    img_window->CropToSequence(blobs);
    img_window->DrawBlobs();
						   ...
						   int g_ind, wxSizer *sizer, int ind)
{
  ...
  m_timeline = new TimelineWindow(panel, this);
...
void LabelFrame::ProcessDirectory(const char *dirName) {
  ...
  save_multi_blob_sequence(all_blobs, behaviors, params.num_spine_lines+1, params.num_orientations);
  for (int i = 0; i < all_blobs->num_blobs; i++) {
    Open(all_blobs->blobs[i]->fname);
    Fit(); 
    predict_all_behaviors(blobs, behaviors);
    SetDirty(true);
    SetFrame(frame);
...
TimelineWindow::TimelineWindow(wxWindow *w,  BehaviorTimelineControls *pa, wxSize sz, long style) : 
...
void TimelineWindow::RecomputeImage(bool get_lock) { 
  ...
  while(i < b->num_frames) {
    ...
    j = i;
    int beh = b->frames[i].behaviors[group_ind];
    j = i;
    while(j < b->num_frames && beh == b->frames[j].behaviors[group_ind])
      j++;
