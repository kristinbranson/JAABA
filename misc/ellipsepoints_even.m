...
function [X,Y,theta] = ellipsepoints_even(a,b,x0,y0,phi,npoints)
...
%----------------------------------------
 ...
 p = [x;y]';     
 q = curvspace(p,npoints); 
 x = q(:,1);
 y = q(:,2);
 
 nm = a.*y;
 dm = b.*x;
 thetanew = atan2(nm,dm);
 
 % debuging
%  figure, plot(x,y,'.'), axis equal, hold on
%  tempx = a* cos(thetanew);
%  tempy = b* sin(thetanew); 
%  plot([tempx],[tempy],'mo')
% change range to 0 : pi, 0 = nose, pi = tail 
 theta = abs(thetanew); 
